        ...
        if (file->options.tracing_enabled) {
            uint32_t crc32 =
                    get_checksum(reinterpret_cast<uint8_t*>(buf->buf + buf_pos),
                                 block_remain,
                                 CRC32C);
            TRACE_INSTANT2("couchstore_write", "raw_write", "offset", write_pos,
                       "nbytes&CRC", block_remain<<32|crc32);
        }
...
couchstore_error_t write_header(tree_file *file, sized_buf *buf, cs_off_t *pos)
{
    ...
    *pos = write_pos;
    ...
    if (written < 0) {
        if (file->options.tracing_enabled) {
            TRACE_INSTANT1("couchstore_write", "write_header", "written", write_pos);
        }
